...
def regression_results(y_true, y_pred):
    ...
    mean_absolute_error=np.mean(errors**2)
    mse=metrics.mean_squared_error(y_true, y_pred) 
    r2=metrics.r2_score(y_true, y_pred)
    mape = 100 * (errors / np.array(y_true).reshape(-1))
    mape = np.mean(mape)
    
    return round(explained_variance,4),  round(r2,4), round(mean_absolute_error,4),round(np.sqrt(mse),4),  round(mape,4)
...
def RFECV_PLSR(X,Y, step, cv, estimator):
    ...
    for n_features_to_select in range(1, X.shape[1] + 1, step):
        rfe = RFE(estimator=PLSRegression(n_components=n_features_to_select), n_features_to_select=n_features_to_select, step=step)
        rfe.fit(X, Y)
    ...
    estimator_ = PLSRegression(n_components=n_features_)#clone(estimator)
